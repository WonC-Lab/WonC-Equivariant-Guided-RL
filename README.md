...
$[\rho_{\text{in}}(g) \cdot s](p) = \rho_{\text{chan}}(g) s(g^{-1} \cdot p)$
...
$[f * \psi](g) = \sum_{h \in G} \psi(h^{-1} g) f(h)$
Let $\rho_L(g')$ be the left regular representation acting on $f$, defined by $[\rho_L(g') \cdot f](g) = f(g'^{-1} g)$. Then the group convolution operation is equivariant with respect to $\rho_L$:
$[(\rho_L(g') \cdot f) * \psi] = \rho_L(g') \cdot [f * \psi]$

*Proof:*
Expanding the definition of group convolution under the transformed input $\rho_L(g') \cdot f$:
$[(\rho_L(g') \cdot f) * \psi](g) = \sum_{h \in G} \psi(h^{-1} g) [\rho_L(g') \cdot f](h) = \sum_{h \in G} \psi(h^{-1} g) f(g'^{-1} h)$
...
$[(\rho_L(g') \cdot f) * \psi](g) = \sum_{h' \in G} \psi((g' h')^{-1} g) f(h') = \sum_{h' \in G} \psi(h'^{-1} g'^{-1} g) f(h')$
By definition of group convolution evaluated at the element $g'^{-1}g$, we obtain:
$= [f * \psi](g'^{-1} g) = [\rho_L(g') \cdot (f * \psi)](g)$
Thus, $[(\rho_L(g') \cdot f) * \psi] = \rho_L(g') \cdot [f * \psi]$ holds for all $g \in G$, proving that group convolution is equivariant under the group action. $\blacksquare$

...
We evaluated the trained $D_4$-Net, a standard CNN, and a standard CNN trained with 8x data augmentation on the 8 elements of the Dihedral Group $D_4$ (rotations and reflections).
The results in Fig.~\ref{fig:generalization} show that $D_4$-Net maintains robust success rates across all symmetric variations ($g_0$ to $g_7$). In contrast, the standard CNN without augmentation fails on rotated configurations. While 8x data augmentation improves the standard CNN's generalizability, it still performs worse than $D_4$-Net and incurs significant data overhead, validating that hardcoded equivariance offers superior sample-free generalization.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{generalization_test.png}
\caption{Zero-Shot Generalization under $D_4$ group transformations.}
\label{fig:generalization}
\end{figure}

\subsection{Exploration Safety Analysis}
Early-stage exploration safety is a major bottleneck in reinforcement learning navigation. We evaluated the cumulative collisions during training with heuristic guidance ($\beta_{start} = 1.0$) and without ($\beta_{start} = 0.0$).
Our results in Fig.~\ref{fig:safety} demonstrate that Heuristic-Guided RL reduces cumulative collisions in early episodes from 160 down to 39 (a $4.1\times$ safety improvement). The KL-divergence regularization restricts the policy search space to safe pathfinding trajectories initially, gradually decaying as the agent becomes competent.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{exploration_safety.png}
\caption{Cumulative obstacle collisions during training with and without heuristic guidance.}
\label{fig:safety}
\end{figure}
...
The proposed $D_4$-Net achieves superior success rates with only $12.5\%$ of the training samples of standard baselines, validating our claims of up to an 8x increase in sample efficiency (Fig.~\ref{fig:sample_eff}).

\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{sample_efficiency.png}
\caption{Sample efficiency and training convergence comparison.}

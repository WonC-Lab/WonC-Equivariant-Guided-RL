...
A neural network layer $f: X \to Y$ is \textbf{equivariant} under a group $G$ if:
\begin{equation}
    f(g \cdot x) = g \cdot f(x) \quad \forall g \in G
\end{equation}
If $f$ maps to a scalar invariant under transformations (such as the state value $v$), it is \textbf{invariant}:
\begin{equation}
...
We formalize and prove this property in the following theorem:

\begin{theorem}[Equivariance of FCN Policy Head]
Let $f_\theta: \mathcal{S} \to \mathbb{R}^{|\mathcal{A}| \times H \times W}$ be a $D_4$-equivariant fully convolutional neural network mapping state grids $s \in \mathcal{S}$ to feature maps. If the policy distribution $\pi_\theta(a|s)$ is constructed via a localized spatial action mapping such that action $a \in \mathcal{A}$ corresponds to a directional movement represented by a vector $d_a$, and we apply the corresponding group permutation $g \cdot a$, then the policy head is equivariant under $D_4$:
\begin{equation}
...
\end{equation}
When the state is transformed by $g$, the agent's position becomes $g \cdot p_{\text{agent}}$. The equivariant features at the new state are $[g \cdot \mathbf{F}]_{a, g \cdot p_{\text{agent}}}$. Because the channels of $\text{EquiConv}$ transform according to the action permutation of $D_4$, we have $[g \cdot \mathbf{F}]_{g \cdot a, g \cdot p_{\text{agent}}} = \mathbf{F}_{a, p_{\text{agent}}}$. Applying the softmax over the action space yields:
\begin{equation}
    \pi_\theta(g \cdot a \mid g \cdot s) = \frac{\exp([g \cdot \mathbf{F}]_{g \cdot a, g \cdot p_{\text{agent}}})}{\sum_{a' \in \mathcal{A}} \exp([g \cdot \mathbf{F}]_{g \cdot a', g \cdot p_{\text{agent}}})} = \frac{\exp(\mathbf{F}_{a, p_{\text{agent}}})}{\sum_{a' \in \mathcal{A}} \exp(\mathbf{F}_{a', p_{\text{agent}}})} = \pi_\theta(a \mid s)
\end{equation}
...
\begin{equation}
    v = V_\theta(s_L)
\end{equation}
The state value $v$ is backpropagated up the tree, updating visit counts $N(s, a)$ and action-values $Q(s, a)$. Crucially, because navigation is modeled as a single-agent MDP, state value backpropagation does not alternate signs, meaning the update is cumulative:
\begin{equation}
...
\end{equation}
Here, $A_i = G_i - V_\theta(s_i)$ is the advantage calculated using deep value bootstrapping (critic) instead of raw returns $G_i$, and $L_V(\theta) = \frac{1}{B} \sum_{i=1}^B (G_i - V_\theta(s_i))^2$ represents the Mean Squared Error (MSE) value head loss. The KL term is:
\begin{equation}
...
We derive the theoretical gradient update rule for our proposed loss function below:

\begin{proof}[Derivation of the Loss Gradient]
Expanding the KL divergence term, we write:
\begin{equation}
...
\end{equation}
where $\mathbb{I}$ is the indicator function. This highlights that the heuristic distribution $P_H(a|s)$ acts as a targeted pseudo-advantage, steering the policy updates towards safe movements during early exploration, proportional to $\beta P_H(a|s)$.
\end{proof}

\subsection{Algorithm \& Training Pipeline}
The integration of our modules forms a cohesive training loop outlined in Algorithm \ref{alg:training_pipeline}.

\begin{algorithm}[H]
\caption{Heuristic-Guided Actor-Critic MCTS Training}
\label{alg:training_pipeline}
\begin{algorithmic}[1]
\REQUIRE Heuristic navigation policy $P_H(s)$, discount factor $\gamma$, decay rate $\gamma_{\text{decay}}$, neural net $f_\theta$, number of simulations $N_{\text{sim}}$
\STATE Initialize weight parameter $\beta \leftarrow \beta_{\text{start}}$
\FOR{episode $e = 1$ \TO $M$}
    \STATE Initialize env state $s_0$, step counter $t \leftarrow 0$, trajectory $\mathcal{T} \leftarrow \emptyset$
    \WHILE{not terminal($s_t$) and $t < T_{\text{max}}$}
        \STATE Initialize MCTS root at $s_t$
        \FOR{sim = $1$ \TO $N_{\text{sim}}$}
            \STATE \textbf{Selection}: Find leaf $s_L$ maximizing $Q(s, a) + c_{\text{puct}} P(s, a) \frac{\sqrt{\sum N(s, b)}}{1 + N(s, a)}$
            \STATE \textbf{Evaluation}: Obtain $\pi_\theta(s_L)$ and $V_\theta(s_L)$ from $f_\theta$
            \STATE \textbf{Expansion}: Initialize children nodes with prior probabilities $P(s_L, a) = \pi_\theta(a|s_L)$
            \STATE \textbf{Backpropagation}: Traverse back to root, updating $N(s, a) \leftarrow N(s, a) + 1$, $Q(s, a) \leftarrow Q(s, a) + \frac{V_\theta(s_L) - Q(s, a)}{N(s, a)}$
        \ENDFOR
        \STATE Compute search probability distribution $\pi_{\text{mcts}}(a) \propto N(s_t, a)^{1/\tau}$
        \STATE Sample action $a_t \sim \pi_{\text{mcts}}(\cdot)$, query heuristic policy $P_H(s_t)$
        \STATE Record step data $(s_t, a_t, P_H(s_t), \log \pi_\theta(a_t | s_t))$ in $\mathcal{T}$
        \STATE Step environment: $s_{t+1} \sim \mathcal{P}(s_t, a_t)$
        \STATE $t \leftarrow t + 1$
    \ENDWHILE
    \STATE Compute returns $G_t = \sum_{k=t}^{T} \gamma^{k-t} R_k$ and advantages $A_t = G_t - V_\theta(s_t)$ for each step in $\mathcal{T}$
    \STATE Update network parameters $\theta$ by performing gradient descent on $L(\theta)$
    \STATE Decay guidance weight: $\beta \leftarrow \max(\beta \cdot \beta_{\text{decay}}, \beta_{\text{min}})$
\ENDFOR
\end{algorithmic}
\end{algorithm}

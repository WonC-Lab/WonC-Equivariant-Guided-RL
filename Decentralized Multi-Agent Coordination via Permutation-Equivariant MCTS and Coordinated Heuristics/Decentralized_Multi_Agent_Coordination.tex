...
%===============================================================================
We presented a decentralized multi-agent coordination framework combining permutation-equivariant GNN architectures with Actor-Critic MCTS. Our experiments with randomised start/goal positions ($N=20$ per MCTS condition, $N=50$ for fast baselines, $N_{\rm search}=20$) yield four concrete empirical findings:
\begin{enumerate}
    \item \textbf{MCTS is non-negotiable}: GNN+1-step lookahead achieves $\leq 2\%$; MCTS with the same GNN achieves $65$--$100\%$. Multi-step tree search is the essential ingredient.
    \item \textbf{Heuristic prior is necessary}: Below $\beta=0.3$ MCTS collapses to $0\%$---the GNN prior alone cannot maintain goal-directedness.
    \item \textbf{Theorem 4 is empirically validated}: $\hat{d}_{\min}$ decreases from $5.45$ (M=2) to $2.35$ (M=8) while the Theorem 4 bound grows from $0.67$ to $101.2$ and failure rate increases monotonically from $0\%$ to $45\%$.
    \item \textbf{Zero-shot spatial generalisation}: A model trained on $13{\times}13$ achieves $85\%$ on $20{\times}20$ zero-shot (5\% drop), confirming size-invariant feature extraction.
\end{enumerate}
Key limitations:
\begin{itemize}
    \item \textbf{Communication assumption}: Rc sensitivity on $13{\times}13$ shows no degradation because inter-agent distances are $\leq 3$ cells. Stress-testing on larger, sparser grids is required to characterize real Rc sensitivity.
    \item \textbf{Static agent modelling in MCTS}: Treating other agents as static obstacles causes high-$M$ coordination failures. Extending to I-POMDP or joint-MCTS representations would address this.
    \item \textbf{Small-$N$ statistics}: At $N=20$, standard deviations are $\pm 30$--$50\%$ for binary outcomes. Camera-ready results should use $N \geq 100$ with confidence interval reporting.
\end{itemize}
Future work: (i) Rc-sparse attention via graph masking in the Transformer; (ii) mutual agent modelling in MCTS rollouts; (iii) continuous 3D robotic navigation benchmarks.
